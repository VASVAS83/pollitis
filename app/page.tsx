          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((post) => (
              <div key={post.id} className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition">
                ...
                <h3 className="text-xl font-bold mb-2">{post.title || 'Χωρίς τίτλο'}</h3>
                <p className="text-gray-600 text-sm mb-4">{post.summary || 'Περίληψη από AI...'}</p>
                ...
                <div className="flex gap-3 mb-4">
                  <button className="bg-green-100 text-green-700 px-4 py-2 rounded-full text-sm">
                    {current.support} {post.reactions?.support || 0}
                  </button>
                  <button className="bg-yellow-100 text-yellow-700 px-4 py-2 rounded-full text-sm">
                    {current.doubt} {post.reactions?.doubt || 0}
                  </button>
                </div>
                <button className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-3 rounded-xl mb-2">
                  {current.ask_ai}
                </button>
                <input placeholder={current.opinion} className="w-full p-3 border rounded-xl text-sm" />
              </div>
            ))}
          </div>
        )}
